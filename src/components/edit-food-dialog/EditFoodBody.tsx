...
const EditFoodBody = ({
    ...
    const [calories, setCalories] = useState<string>(
        food.type === FoodType.BY_WEIGHT
            ? (food.calories_p100?.toString() || '')
            : (food.calories_fixed?.toString() || ''),
    );
    const [servingSize, setServingSize] = useState<string>(food.serving_grams?.toString() || '100');
    const [servingType, setServingType] = useState<FoodType>(food.type);
    ...
    return (
        <>
            ...
            <DialogActions>
                <Button variant="contained" disabled={!isValidCalorieValue(calories)} onClick={() => updateEntry()}>Save</Button>
